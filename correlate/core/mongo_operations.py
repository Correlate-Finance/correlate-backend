...
def fetch_category_names(db):
    ...
    category_data = {document['name']: document['_id'] for document in category_documents}
...
def fetch_data_table_ids(db, selected_name=None):
    ...
    if selected_name is not None:
        ...
        dataTable_documents = dataTable_documents.find({"category": selected_id}, {"title": 1, "_id": 1})
    else:
...
def fetch_data_frames(db, dataTable_ids, date_threshold=datetime.strptime("1971-01-01", "%Y-%m-%d"), max=None):
    ...
    for id_as_string, title in dataTable_ids.items():
        pipeline = [
            {"$match": {"dataTable_Id": id_as_string, "date": {"$gte": date_threshold}}},
